...
export default function Categories() {
  ...
  const saveMutation = useMutation({
    mutationFn: (d) => editing ? api.put(`/categories/${editing._id}`, d) : api.post('/categories', d),
    onSuccess: () => { qc.invalidateQueries(['categories']); setShowModal(false); setEditing(null); setForm({ name: '', description: '' }) }
  })
  ...
  return (
    <div className="p-6 max-w-2xl space-y-5">
        ...
        {isLoading ? (
          ...
          <div key={cat.id} className="flex items-center justify-between px-5 py-3">
            ...
            <div className="flex gap-2">
              <button onClick={() => { setEditing(cat); setForm({ name: cat.name, description: cat.description || '' }); setShowModal(true) }}
      ...
      <Modal open={showModal} onClose={() => { setShowModal(false); setEditing(null) }} title={editing ? 'Edit Category' : 'New Category'} size="sm">
